...
По умолчанию - `1341`. Номер UDP порта, используемый QuickConfigUtility.
...
По умолчанию - `6000`. Номер UDP порта (`netstat -oan` позволил выяснить данный факт),
на который Server Manager принимает широковещательные пакеты от серверов USB
...
Вероятнее всего, этот порт открывается опять-таки на USB сервере, а клиент использует
динамический TCP порт. Но данное предположение не подтверждено.

#### DEVICE_REQUEST_PORTNO

По умолчанию - `6133`. Входящий TCP (UDP?) порт, откываемый на клиенте приложением Server Manager.
