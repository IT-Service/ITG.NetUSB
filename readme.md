...
По умолчанию - `0`. `1` означает, что все подключенные в будущем к данному порту NetUSB устройства
будут автоматически подключенны к локальной машине. Естественно, данный режим может быть установлен
только на одном ПК для каждого конкретного порта NetUSB.
