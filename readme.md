...
По умолчанию - `1`. Означает, что все подключенные в будущем к данному порту NetUSB устройства
будут автоматически переподключаться при перезапуске компьютера / службы / приложения. Этот параметр
можно изменить для каждого конкретного устройства при его подключении.
