...
P.S. Текущая редакция требует WiX 4.0. 
...
По умолчанию - `0`. Означает, что все подключенные в будущем к NetUSB устройства
автоматически подключаться к клиенту не будут, необходимо будет их явное подключение. Но это не означает,
что подключенные устройства не будут автоматически переподключаться при потери связи / перезагрузке
(см. [DEFAULT_AUTORECONECT_VALUE](#default_autoreconect_value)).

Устанавливать данный параметр в `1` следует только в том случае, если у Вас всего 1 клиент планируется,
в противном случае "угадать" к какому клиенту с установленным в `1` данным флагом будет подключено новое
устройство не представляется возможным.

#### DEFAULT_AUTORECONECT_VALUE

По умолчанию - `1`, что означает, что при временной потере подключения
и при перезагрузке клиента / перезапуске службы
подключенные ранее USB устройства будут переподключаться автоматически.
...
`0` - Служба автоматически запускаться не будет.

P.S. На текущий момент данный параметр не оказывает никакого влияния на процесс установки:
служба регистрируется и запускается автоматически.
...
По умолчанию - `1`. Означает, что будут задействованы механизмы Windows 7 для выделения сетевого канала
с гарантированной пропускной способностью, чтобы исключить возможные проблемы с подключенными USB устройствами,
связанные с загруженностью сетевого подключения другими приложениями.

'0' - указанные выше механизмы задействованы не будут.

Поддерживается только на Windows 7 и старше.

#### LOGFILEFOLDER

По умолчанию - `C:\Windows\Logs\UsbIp`, и полный путь к журналу - `C:\Windows\Logs\UsbIp\USBIPAdmin.log`.
Вы можете задать иной путь к журналу либо при создании административной точки установки, либо
трансформацией, используя описываемое свойство.
...
Комментарии по работе с приложением USB over IP client
------------------------------------------------------

В процессе установки регистрируется служба `UsbIpService`. Само приложение следует запускать 
с правами администратора, в противном случае подключение устройств и управление USB серверами 
будут недоступны.

Для успешного подключения устройств приложение должно предварительно остановить указанную службу.
Если по какой-то причине приложение выдаёт ошибку о невозможности отключения функции отображения
устройств (device mapping), остановите службу самостоятельно:

	net stop UsbIpService

По завершению конфигурирования снова остановите функцию Device Mapping, завершите работу приложения 
(меню Файл, Выход), и запустите службу:

	net start UsbIpService

Для автоматического подключения ранее подключенных устройств при перезагрузке / перезапуске службы
неоходимо для этих устройств установить флаги `auto connect`, `auto reconnect`. В этом случае эти
устройства будут подключены автоматически даже после переустановки ПО (причём - сразу после
переустановки).

P.S. Служба не запустится, если хотя бы один экземпляр приложения выполняется. Поэтому предварительно
убедитесь, что Вы завершили работу приложения, а не свернули его (значок приложения "сворачивается" в
область уведомлений).
